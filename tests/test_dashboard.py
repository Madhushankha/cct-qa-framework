...
def test_parse_cell():
    assert _parse_cell("int_brove_fd_204833") == {"env": "int", "product": "brove", "feed": "fd", "time": "204833"}


def test_collect_and_build(tmp_path):
    _run(tmp_path, "2026-07-16", "int_brove_fd_100000", [True, True, False], docs=["index.html"])
    _run(tmp_path, "2026-07-16", "int_brove_fd_090000", [True])
    _run(tmp_path, "2026-07-15", "crt_brove_soc_120000", [False])
    ...
    assert "int · brove · fd" in html
    assert '2026-07-16/int_brove_fd_100000/index.html' in html  # links to per-run report
    assert "2026-07-15" in html and "2026-07-16" in html


def test_chips_link_only_when_files_exist(tmp_path):
    _run(tmp_path, "2026-07-16", "int_brove_fd_100000", [True],
    ...
    assert '<a class="chip" href="2026-07-16/int_brove_fd_100000/index.html">Run</a>' in html
    assert '<a class="chip" href="2026-07-16/int_brove_fd_100000/quality-index.html">Quality</a>' in html
...
def test_dead_run_dir_is_not_linked(tmp_path):
    ...
    _run(tmp_path, "2026-07-16", "int_brove_fd_090000", [True])
    html = build_dashboard(tmp_path).read_text(encoding="utf-8")
    assert 'href="2026-07-16/int_brove_fd_090000/index.html"' not in html
    assert '<span class="chip off">Run</span>' in html


def test_rows_carry_data_attributes(tmp_path):
    _run(tmp_path, "2026-07-16", "int_brove_fd_100000", [True, True, False])
    html = build_dashboard(tmp_path).read_text(encoding="utf-8")
    assert 'data-env="int"' in html
    assert 'data-product="brove"' in html
...
def test_download_anchors(tmp_path):
    _run(tmp_path, "2026-07-16", "int_brove_fd_100000", [True], docs=["index.html"])
    html = build_dashboard(tmp_path).read_text(encoding="utf-8")
    assert 'download href="2026-07-16/int_brove_fd_100000/index.html"' in html
