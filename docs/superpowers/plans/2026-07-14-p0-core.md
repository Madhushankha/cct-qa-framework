...
- Create/overwrite: `core/registry/products/brove.yaml`
...
`core/registry/products/brove.yaml`:
```yaml
id: brove
label: Brove
transcript_dialect: brove
overrides:
  persona: {}
  judge: {}
defaults:
...
git commit -m "feat(core): descriptor data files (fd/soc feeds, crt/int envs, brove product)"
```
...
- §8 deliverables (real fd/crt/int/brove + one extra feed) → Task 4 adds `soc`. ✓
